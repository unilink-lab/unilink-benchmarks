...
void print_usage(const char* program) {
  std::cerr << "Usage: " << program
            ...
            << "       [--iterations COUNT] [--warmup-iterations COUNT] [--timeout-ms MS]\n"
...
SmokeConfig parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--host" && unilink_bench::read_option(argc, argv, i, &value)) {
      config.host = value;
    } else if (arg == "--port" && unilink_bench::read_option(argc, argv, i, &value)) {
      config.port = unilink_bench::parse_port(value);
    } else if (arg == "--payload-sizes" && unilink_bench::read_option(argc, argv, i, &value)) {
      config.payload_sizes = parse_payload_sizes(value);
    } else if (arg == "--iterations" && unilink_bench::read_option(argc, argv, i, &value)) {
      config.iterations = unilink_bench::parse_size(value, "iterations");
    } else if (arg == "--warmup-iterations" && unilink_bench::read_option(argc, argv, i, &value)) {
      config.warmup_iterations = unilink_bench::parse_nonnegative_size(value, "warmup iterations");
    } else if (arg == "--timeout-ms" && unilink_bench::read_option(argc, argv, i, &value)) {
      config.timeout_ms = unilink_bench::parse_positive_int(value, "timeout");
    } else if (arg == "--strategy" && unilink_bench::read_option(argc, argv, i, &value)) {
      if (value != "reliable" && value != "besteffort" && value != "both") {
        throw std::invalid_argument("strategy must be reliable, besteffort, or both");
      }
      config.strategy = value;
    } else if (arg == "--csv-output" && unilink_bench::read_option(argc, argv, i, &value)) {
...
void append_csv(const std::string& path, const SmokeResult& result) {
  ...
  if (write_header) {
    output << "transport,strategy,payload_size,iterations,warmup_iterations,send_attempts,send_success,"
              "send_failures,server_received,client_received,echo_matches,timeouts,delivery_percent,"
              "match_percent,elapsed_ms,messages_per_sec,notes\n";
  }

  output << "udp," << result.strategy << "," << result.payload_size << "," << result.iterations << ","
         << result.warmup_iterations << "," << result.send_attempts << "," << result.send_success << ","
         << result.send_failures << "," << result.server_received << "," << result.client_received << ","
         << result.echo_matches << "," << result.timeouts << "," << std::fixed << std::setprecision(6)
         << result.delivery_percent << "," << result.match_percent << "," << result.elapsed_ms << ","
         << result.messages_per_sec << "," << result.notes << "\n";
}

SendOutcome send_and_wait(unilink::UdpClient& client, EchoState& state, const std::string& payload,
  ...
  return {true, state.cv.wait_for(lock, timeout, [&state] { return state.received; })};
}

SmokeResult run_payload(unilink::UdpClient& client, EchoState& state,
                        const std::atomic<uint64_t>& server_received_total,
                        const std::atomic<uint64_t>& client_received_total,
                        const std::atomic<uint64_t>& echo_matches_total, Strategy strategy, size_t payload_size,
                        size_t iterations, size_t warmup_iterations, std::chrono::milliseconds timeout) {
  for (size_t i = 0; i < warmup_iterations; ++i) {
    const auto payload = make_payload(payload_size, i);
    (void)send_and_wait(client, state, payload, timeout);
  }

  const auto server_before = server_received_total.load(std::memory_order_relaxed);
  const auto client_before = client_received_total.load(std::memory_order_relaxed);
  const auto matches_before = echo_matches_total.load(std::memory_order_relaxed);
  ...
  result.server_received = server_received_total.load(std::memory_order_relaxed) - server_before;
  result.client_received = client_received_total.load(std::memory_order_relaxed) - client_before;
  result.echo_matches = echo_matches_total.load(std::memory_order_relaxed) - matches_before;

  result.delivery_percent =
      result.send_success == 0 ? 0.0 : 100.0 * static_cast<double>(result.client_received) / result.send_success;
  result.match_percent =
      result.send_success == 0 ? 0.0 : 100.0 * static_cast<double>(result.echo_matches) / result.send_success;
  result.elapsed_ms = std::chrono::duration<double, std::milli>(stop - start).count();
  result.messages_per_sec = result.elapsed_ms <= 0.0 ? 0.0 : 1000.0 * result.echo_matches / result.elapsed_ms;
...
void run_strategy(const SmokeConfig& config, Strategy strategy, uint16_t port) {
  ...
  server->on_error([](const unilink::ErrorContext& ctx) { std::cerr << "[udp smoke server] " << ctx.message() << "\n"; });
  ...
  client.on_error([](const unilink::ErrorContext& ctx) { std::cerr << "[udp smoke client] " << ctx.message() << "\n"; });
  ...
  std::cout << "udp payload smoke: strategy=" << strategy_name(strategy) << " port=" << port << "\n";
  const auto timeout = std::chrono::milliseconds(config.timeout_ms);
  for (const auto payload_size : config.payload_sizes) {
    const auto result = run_payload(client, state, server_received_total, client_received_total, echo_matches_total,
                                    strategy, payload_size, config.iterations, config.warmup_iterations, timeout);
    append_csv(config.csv_output, result);
    std::cout << "  payload=" << std::setw(6) << payload_size << " delivery=" << std::fixed
              << std::setprecision(2) << result.delivery_percent << "% match=" << result.match_percent
