...
SendOutcome send_and_wait(wirestead::UdpClient& client, EchoState& state,
...
SmokeResult run_payload(wirestead::UdpClient& client, wirestead::UdpServer& server,
                        EchoState& state,
...
void run_strategy(const SmokeConfig& config, Strategy strategy, uint16_t port) {
  ...
  std::unique_ptr<wirestead::UdpServer> server;
  server = std::make_unique<wirestead::UdpServer>(port);
  ...
  server->on_error([](const wirestead::ErrorContext& ctx) {
  ...
  client.on_error([](const wirestead::ErrorContext& ctx) {
