...
int main(int argc, char** argv) {
  try {
    const auto port = wirestead_bench::parse_server_port_args(argc, argv, wirestead_bench::kDefaultUdpPort);
    std::unique_ptr<wirestead::UdpServer> server;

    server =
        wirestead::udp_server(port)
    ...
    if (!server->start_sync()) {
      std::cerr << "Failed to start UDP echo server on port " << port << "\n";
      return 1;
    }
