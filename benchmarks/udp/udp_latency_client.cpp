...
class UdpBenchClient {
 public:
  explicit UdpBenchClient(const unilink_bench::NetworkConfig& config) {
    client_ =
 ...
 private:
  ...
  std::unique_ptr<unilink::UdpClient> client_;
};

int main(int argc, char** argv) {
  try {
    const auto config = unilink_bench::parse_network_client_args(argc, argv, unilink_bench::kDefaultUdpPort);
    UdpBenchClient client(config);
    return unilink_bench::run_latency_client("udp", client, config.payload_size, config.iterations,
