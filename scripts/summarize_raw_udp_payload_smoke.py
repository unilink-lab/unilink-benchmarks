...
def build_notes(rows):
    notes = [
        "- raw 4096 succeeds but Wirestead 4096 fails -> likely Wirestead UDP path issue.",
        "- raw 4096 fails and Wirestead 4096 fails -> likely environment / WSL2 / OS behavior.",
        "- raw succeeds and Wirestead succeeds -> strategy pressure model issue.",
    ]
    ...
    for row in rows:
        ...
        if send_success > 0 and server_received == 0:
            notes.append(
                ...
                "socket buffer, or datagram behavior before attributing this to Wirestead."
            )
