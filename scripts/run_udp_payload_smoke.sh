...
OUTPUT="${OUTPUT:-${ROOT_DIR}/build/udp_payload_smoke.csv}"
SUMMARY="${SUMMARY:-${ROOT_DIR}/build/udp_payload_smoke_summary.md}"

mkdir -p "$(dirname "${OUTPUT}")" "$(dirname "${SUMMARY}")"
rm -f "${OUTPUT}" "${SUMMARY}"

"${UNILINK_BENCH_BUILD_DIR}/bin/bench_udp_payload_smoke" \
  --host "${HOST}" \
  --port "${PORT}" \
...
python3 "${ROOT_DIR}/scripts/summarize_udp_payload_smoke.py" \
  --input "${OUTPUT}" \
  --output "${SUMMARY}"
