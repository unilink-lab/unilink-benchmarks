...
def build_notes(result_dir, wirestead_ref, platform_suffix, reference_platform):
    ...
    lines = [
        f"Self-hosted benchmark results for `wirestead` `{wirestead_ref}`.",
        "",
        "## Benchmark Target",
        "",
        f"- wirestead ref: `{wirestead_ref}`",
        f"- wirestead commit: `{value_or_unknown(wirestead_commit)}`",
    ...
    lines.extend(
        ...
        strip_markdown_heading(
            read_section(
                result_dir / "raw_udp_payload_smoke_summary.md",
                "_Raw UDP payload smoke summary was not generated._",
            ),
...
def main():
    ...
    output.write_text(
        build_notes(result_dir, wirestead_ref, args.platform_suffix, args.reference_platform),
        encoding="utf-8",
    )
    print(output)
